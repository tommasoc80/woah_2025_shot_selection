...
#    printing all data for in-context learning
    ...
    top_10_not_hateful.to_csv('brexit_difficult_training_not_first.csv', columns = ['text', 'hard_label'], index=False)
    top_10_hateful.to_csv('brexit_difficult_training_not_first.csv', columns = ['text', 'hard_label'], index=False, mode='a', header=False)
    top_difficult_merge = pd.concat([top_10_not_hateful,top_10_hateful]).sample(frac=1)
    #top_difficult_merge.sample(frac=1)
    top_difficult_merge.to_csv('brexit_difficult_training_shuffled.csv', columns = ['text', 'hard_label'], index=False)

    ## ambiguous examples

    random_ambiguous_not_hateful_10.to_csv('brexit_ambiguous_training_not_first.csv', columns = ['text', 'hard_label'], index=False)
    random_ambiguous_hateful_10.to_csv('brexit_ambiguous_training_not_first.csv', columns = ['text', 'hard_label'], index=False, mode='a', header=False)
    ambiguos_merge = pd.concat([random_ambiguous_not_hateful_10,random_ambiguous_hateful_10]).sample(frac=1)
    ambiguos_merge.to_csv('brexit_ambiguous_training_shuffled.csv', columns = ['text', 'hard_label'], index=False)
    ## random examples

    random_not_hateful_10.to_csv('brexit_random_training_not_first.csv', columns = ['text', 'hard_label'], index=False)
    random_hateful_10.to_csv('brexit_random_training_not_first.csv', columns = ['text', 'hard_label'], index=False, mode='a', header=False)
    random_merge = pd.concat([random_not_hateful_10,random_hateful_10]).sample(frac=1)
    random_merge.to_csv('brexit_random_training_shuffled.csv', columns = ['text', 'hard_label'], index=False)
...
if __name__ == '__main__':
    ...
    brexit_train = sys.argv[2] # file: ./aggregated_split/Brexist_hard_label_train.csv
    retrieve_ids(mace_entropies, brexit_train)
