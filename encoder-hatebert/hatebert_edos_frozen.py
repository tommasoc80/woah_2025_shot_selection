...
def train_model(tokenized_dataset, data_collator):
    ...
    training_args = TrainingArguments(
        output_dir="/scratch/p281734/edos_frozen",
        learning_rate=2e-5,
...
if __name__ == '__main__':
    ...
    train = pd.read_csv('/scratch/p281734/edos_aggregated_train.csv', delimiter=',', header=0)
    dev = pd.read_csv('/scratch/p281734/edos_aggregated_dev.csv', delimiter=',', header=0)
    test = pd.read_csv('/scratch/p281734/edos_aggregated_test.csv', delimiter=',', header=0)

    train_clean = clean_samples(train)
    train_clean.drop(columns="text", inplace=True)
    train_clean.rename(columns={"cleaned_text": "text"}, inplace=True)

    dev_clean = clean_samples(dev)
    dev_clean.drop(columns="text", inplace=True)
    dev_clean.rename(columns={"cleaned_text": "text"}, inplace=True)

    test_clean = clean_samples(test)
    test_clean.drop(columns="text", inplace=True)
    test_clean.rename(columns={"cleaned_text": "text"}, inplace=True)
    ...
    print("Evaluation EDOS Frozen - Test data:")
    print()
    print(classification_report(test_clean["label"], predictions, digits=4))

    test_clean['predictions'] = predictions
    test_clean.to_csv('/scratch/p281734/edos_hb_frozen.csv', index=False)
