...
def train_model(tokenized_dataset, data_collator):
    ...
    training_args = TrainingArguments(
        output_dir="/scratch/p281734/gab_frozen",
        learning_rate=2e-5,
...
if __name__ == '__main__':
    ...
    train = pd.read_csv('/scratch/p281734/ghc_train_20250307.csv', delimiter=',', header=0)
    dev = pd.read_csv('/scratch/p281734/ghc_dev_20250307.csv', delimiter=',', header=0)
    test = pd.read_csv('/scratch/p281734/ghc_test_20250307.csv', delimiter=',', header=0)

    train_clean = clean_samples(train)
    train_clean.drop(columns="text", inplace=True)
    train_clean.rename(columns={"gold_hate": "label", "cleaned_text": "text"}, inplace=True)

    dev_clean = clean_samples(dev)
    dev_clean.drop(columns="text", inplace=True)
    dev_clean.rename(columns={"gold_hate": "label", "cleaned_text": "text"}, inplace=True)

    test_clean = clean_samples(test)
    test_clean.drop(columns="text", inplace=True)
    test_clean.rename(columns={"gold_hate": "label", "cleaned_text": "text"}, inplace=True)
    ...
    print("Evaluation GAB Frozen - Test data:")
    print()
    print(classification_report(test_clean["label"], predictions, digits=4))

    test_clean['predictions'] = predictions
    test_clean.to_csv('/scratch/p281734/gab_hb_frozen.csv', index=False)
